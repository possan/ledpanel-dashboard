...
PixelBuffer.prototype.clear = function() {
  for(var o=0; o<this.width * this.height; o++) {
    this.pixels[o] = 0;
  }
}
...
PixelBuffer.prototype.setPixel = function(x, y, on) {
  ...
  if (typeof(on) == 'boolean')
    on = on ? 255 : 0;
  this.pixels[Math.floor(y) * this.width + Math.floor(x)] = on;
}
...
PixelBuffer.prototype.downsampleFrom = function(other) {
  for(var y=0; y<this.height; y++) {
    for(var x=0; x<this.width; x++) {
      var oldpixel1 = other.getPixel(x*2+0, y*2+0);
      var oldpixel2 = other.getPixel(x*2+0, y*2+1);
      var oldpixel3 = other.getPixel(x*2+1, y*2+0);
      var oldpixel4 = other.getPixel(x*2+1, y*2+1);
      var newpixel = (oldpixel1 + oldpixel2 + oldpixel3 + oldpixel4) / 4;
      this.setPixel(x, y, newpixel);
    }
  }
}
